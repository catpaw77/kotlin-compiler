...
public:
	...
	bool isConst(string id) {
		if(stb.back().isConst(id)){
		return stb.back().isConst(id);
		}
		return stb.front().isConst(id);
	}
	bool isInit(string id) {
	...
	int getIndex(string id){
		if (stb.back().id!="main"){
			return stb.back().getIndex(id);
		}
		else{
			return stb.back().getIndex(id)+1;
		}
	}
